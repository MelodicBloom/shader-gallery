...
function normalizeFragmentSource(source) {
  ...
  const declarations = [];
  if (!/precision\s+(lowp|mediump|highp)\s+float\s*;/.test(normalized)) declarations.push("precision highp float;");
  if (!/\bin\s+vec2\s+v_uv\s*;/.test(normalized)) declarations.push("in vec2 v_uv;");
  if (!/\bout\s+vec4\s+outColor\s*;/.test(normalized)) declarations.push("out vec4 outColor;");
  if (!/\buniform\s+float\s+u_time\s*;/.test(normalized)) declarations.push("uniform float u_time;");
  if (!/\buniform\s+vec2\s+u_resolution\s*;/.test(normalized)) declarations.push("uniform vec2 u_resolution;");
  if (!/\buniform\s+vec2\s+u_mouse\s*;/.test(normalized)) declarations.push("uniform vec2 u_mouse;");
...
export class ShaderPlayer {
  constructor(canvas, shader, options = {}) {
    if (typeof HTMLCanvasElement === "undefined" || !(canvas instanceof HTMLCanvasElement)) throw new TypeError("ShaderPlayer requires an HTMLCanvasElement.");
    this.canvas = canvas;
    this.shaderSourceOrUrl = shader;
    this.options = { pixelRatio: browserPixelRatio(), alpha: true, antialias: true, preserveDrawingBuffer: true, premultipliedAlpha: false, autoResize: true, trackPointer: true, autoStart: false, ...options };
    this.gl = null;
    this.program = null;
  ...
  async init() {
    this.gl = this.canvas.getContext("webgl2", { alpha: this.options.alpha, antialias: this.options.antialias, preserveDrawingBuffer: this.options.preserveDrawingBuffer, premultipliedAlpha: this.options.premultipliedAlpha });
    if (!this.gl) throw new Error("WebGL2 is unavailable. Use a WebGL2-capable browser or provide a static preview.");
  ...
  async replaceShader(shader, { restart = this.running } = {}) {
    ...
    this.shaderSourceOrUrl = shader;
    this.#cacheUniforms();
    for (const [name, value] of this.values) this.setUniform(name, value);
  ...
  #cacheUniforms() {
    ...
    for (let index = 0; index < count; index += 1) {
      ...
      this.uniforms.set(name,{ location: gl.getUniformLocation(this.program,name), glType: info.type, type: GLSL_TYPES[info.type] || "unknown", size: info.size });
    }
  }
  ...
  #installObservers() {
    ...
    if (this.options.trackPointer) {
      this.canvas.addEventListener("pointermove",event => {
        ...
        this.mouse = [(event.clientX - rect.left) / rect.width, 1 - (event.clientY - rect.top) / rect.height];
      },{ signal });
    }
  }
  ...
  renderOnce({ time = this.fixedTime ?? 0, clear = true } = {}) {
    if (!this.gl || !this.program || !this.quad) throw new Error("Call init() before rendering.");
    const gl = this.gl;
    ...
    for (const [name,value] of this.values) if (!["u_time","u_resolution","u_mouse","u_res"].includes(name)) this.setUniform(name,value);
  ...
  setFixedTime(time = null) { this.fixedTime = time === null ? null : Number(time); return this; }
  captureDataURL(type = "image/png", quality = 0.92) { return this.canvas.toDataURL(type,quality); }
  captureFrame(type = "image/png", quality = 0.92) {
    return new Promise((resolve,reject) => this.canvas.toBlob(blob => blob ? resolve(blob) : reject(new Error("Canvas capture returned no data.")),type,quality));
  }
...
export async function preview(canvas,shader,options = {}) {
  const player = new ShaderPlayer(canvas,shader,{ autoResize: false, trackPointer: false, preserveDrawingBuffer: true, pixelRatio: 1, ...options });
  await player.init();
